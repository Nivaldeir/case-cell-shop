...
import { EntryDebitBalance } from "@/application/usecase/Account/EntryDebitBalance";
...
function makeBalance(available = 200_00) {
...
describe("EntryDebitBalance — validações de entrada", () => {
	...
	it("lança erro quando amount é negativo", async () => {
		const sut = makeSut();
		await assert.rejects(
			() => sut.execute({ accountId: "acc_001", amount: -50 }),
			(e: Error) => e.message === "Amount must be greater than zero",
		);
	});
});

describe("EntryDebitBalance — validação de conta", () => {
	it("lança AccountNotFound quando conta não existe", async () => {
		const sut = makeSut({ accountRepo: makeAccountRepo(null as any) });
		await assert.rejects(
			() => sut.execute({ accountId: "acc_001", amount: 50_00 }),
			(e) => e instanceof AccountNotFound,
		);
	});
});

describe("EntryDebitBalance — validação de saldo", () => {
	...
	it("lança BalanceInsufficient quando saldo disponível é menor ou igual ao amount", async () => {
		const sut = makeSut({ balanceRepo: makeBalanceRepo(makeBalance(50_00)) });
		await assert.rejects(
			() => sut.execute({ accountId: "acc_001", amount: 50_00 }),
	...
	it("lança BalanceInsufficient quando saldo é zero", async () => {
		const sut = makeSut({ balanceRepo: makeBalanceRepo(makeBalance(0)) });
		await assert.rejects(
			() => sut.execute({ accountId: "acc_001", amount: 10_00 }),
			(e) => e instanceof BalanceInsufficient,
		);
	});

	it("não lança quando saldo disponível é maior que amount", async () => {
		const sut = makeSut({ balanceRepo: makeBalanceRepo(makeBalance(200_00)) });
	...
	it("lança erro de optimistic lock quando updateTx retorna null", async () => {
		const sut = makeSut({
			balanceRepo: makeBalanceRepo(makeBalance(200_00), {
				updateTx: async () => null,
			}),
		});
		await assert.rejects(
...
describe("EntryDebitBalance — output", () => {
	...
	it("entry tem direction=debit, type=adjustment, assetCode=BRL", async () => {
		const sut = makeSut();
		const result = await sut.execute({ accountId: "acc_001", amount: 75_00 });

		const entry = result.entries[0];
		assert.equal(entry.direction, "debit");
		assert.equal(entry.type, "adjustment");
		assert.equal(entry.assetCode, "BRL");
		assert.equal(entry.amount, 75_00);
	});
